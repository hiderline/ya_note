...
# Проверям, кто может отправить POST-запрос для создания заметки.
# В запросе должна быть отправлена форма с заполненными полями;
# данные для формы сохраним в фикстуры.

# В тестах надо отправить POST-запрос на создание заметки
# от имени залогиненного и анонимного пользователей;
# но дальнейшие проверки будут сильно отличаться:
    # залогиненного пользователя должно переадресовать на страницу успешного
    # создания заметки;
    # нужно проверить, что заметка добавлена в БД и
    # что значения её полей соответствуют ожиданиям;
...
# Залогиненный юзер может создать заметку:
...
# Проверяем возможность создать две заметки с одинаковым slug:
...
# Проверка, что автор может редактировать заметку:
